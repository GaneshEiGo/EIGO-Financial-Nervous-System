...
def fetch_data():
    ...
    data = yf.download(tickers, start=CONFIG["start_date"], end=end_date)["Adj Close"]
    return data.dropna()
...
def compute_fragility(data):
    ...
    corr_matrix = returns.rolling(30).corr().groupby(level=0).mean().mean(axis=1)

    credit_spread = (data["HYG"] / data["LQD"]).pct_change().rolling(30).mean()

    fragility = (volatility.rank(pct=True) +
                 corr_matrix.rank(pct=True) +
                 credit_spread.rank(pct=True)) / 3

    fragility = fragility * 100

    return fragility.dropna()
